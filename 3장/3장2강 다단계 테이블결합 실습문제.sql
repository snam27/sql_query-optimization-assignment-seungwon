...
9. 다음 질문에 답하세요.
   Q1. shipments를 INNER JOIN이 아니라 LEFT JOIN으로 연결해야 하는 이유는 무엇인가요?
    - INNER JOIN을 쓰면 shipments에 매칭되는 행이 없는 주문은 결과에서 통째로 사라져 orders 전체 건수를 보장할 수 없기 때문이다. 
    
   Q2. shipment_id가 NULL인 행은 어떤 의미인가요?
   	- 해당 order_id에 대응하는 배송 레코드가 shipments에 존재하지 않는다는 뜻, 즉 아직 배송이 등록되지 않은(또는 누락된) 주문이다.
   
   Q3. 조인 후 행 수가 orders보다 많아졌다면 어떤 관계나 데이터를 먼저 점검해야 하나요?
    - shipments 테이블에서 하나의 order_id가 여러 행으로 존재하는지부터 점검해야 한다.
   
   Q4. 조인 결과 검증을 위해 행 수, 중복, NULL을 함께 확인해야 하는 이유는 무엇인가요?
    - 행 수만 보면 뻥튀기(중복)는 잡아도 NULL(매칭 누락)은 못 잡고, 
      NULL만 보면 중복은 못 잡기 때문에 세 가지를 함께 봐야 조인 결과의 정합성을 온전히 검증할 수 있다.
   
   Q5. LEFT JOIN한 shipments의 status 조건을
       ON 절에 작성하는 경우와 WHERE 절에 작성하는 경우
       결과가 어떻게 달라질 수 있나요?
    - ON 절에 넣으면 조건에 안 맞는 shipments 행은 NULL로 채워지되 orders 행은 그대로 유지되지만, 
      WHERE 절에 넣으면 LEFT JOIN 자체가 사실상 INNER JOIN처럼 동작해 조건에 안 맞는 주문 행이 결과에서 제거된다.
...
-- 배송 정보 없는 주문 수
SELECT COUNT(*) AS null_shipment_count
FROM orders o
LEFT JOIN shipments s ON o.order_id = s.order_id
