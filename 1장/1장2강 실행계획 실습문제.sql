...
5. 다음 항목을 확인해 기록하세요.
   - 스캔 방식: Parallel Seq Scan (+ Gather)
   - cost의 시작 비용: 1000.00
   - cost의 총 비용: 7953.00
   - 예상 rows: 60
   - actual rows: 48
   - Rows Removed by Filter: 199984
   - Execution Time: 112.791ms
   
6. 예상 rows와 actual rows를 비교하여
   옵티마이저의 예상이 실제 결과와 어느 정도 일치하는지 설명하세요.
   - 예상 row는 60이고, actual rows는 48로 옵티마이저의 예상이 
   어느정도 근접하다고 할 수 있다. 
   
7. 다음 질문에 답하세요.
   Q1. 가장 먼저 확인해야 할 데이터 접근 방식은 무엇인가요?
   - 실행계획에서 가장 먼저 확인해야 할 것은 스캔 방식입니다. 
     테이블에 접근할 때 Seq Scan을 쓰는지, Index Scan/Index Only Scan을 쓰는지를 확인해야 합니다.
   
   Q2. Seq Scan은 테이블의 데이터를 어떤 방식으로 확인하나요?
   - Seq Scan(순차 스캔)은 테이블의 첫 페이지부터 마지막 페이지까지 디스크에 저장된 순서대로 모든 행을 하나씩 차례로 읽으면서, 
     각 행이 WHERE 조건(product_id = 100)을 만족하는지 검사하는 방식입니다.
