...
6. 각 customer_group별 고객 수를 확인하세요.
  - group1,2,3의 고객수는 12438명, group4의 고객수는 12437명이다.
  
7. 각 customer_group별 평균 구매금액을 계산하세요.
  - 그룹별 평균 구매금액을 정수로 반올림 했을떼, 
    group1은 133,349원, group2는 85,903원, group3은 59,035원, group4는 29,461원이다.
...
8. 다음 질문에 답하세요.
   ...
   - 내림차순으로 정렬했으므로 총 구매금액이 가장 높은 상위 고객군으로 해석할 수 있다. 
   Q3. 이 결과를 마케팅 업무에 어떻게 활용할 수 있나요?
   - VIP 등급별 차등 프로모션, 이탈 위험이 낮은 상위 그룹 리텐션 관리, 
     하위 그룹 대상 구매 유도 캠페인 등 타겟팅 마케팅에 활용할 수 있다.
...
SELECT customer_id, total_amount, customer_group
FROM customer_groups
ORDER BY customer_group ASC, total_amount DESC;

-- 그룹별 고객수 및 평균 구매금액 확인
WITH customer_totals AS (
    SELECT
        o.customer_id,
        SUM(oi.qty * oi.price) AS total_amount
