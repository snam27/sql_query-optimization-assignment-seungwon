...
8. 인덱스 생성 전후의 스캔 방식과 Execution Time을 비교하세요.
   - 인덱스 생성 전과 생성 후 모두 Seq Scan 방식이 사용되었고 execution time이 0.029ms로 동일했다. 

9. 다음 네 가지 기준으로 stores.city 인덱스의 적절성을 판단하세요.
   - 카디널리티: 4로 매우 낮아 인덱스에 불리하다.
   - 선택도: 21~31%로 매우 낮아 인덱스에 불리하다. 
   - 테이블 크기: 100행, 1페이지 수준으로 테이블이 작다.인덱스의 이점이 별로 없다.
   - 인덱스 유지 비용: INSERT/UPDATE/DELETE마다 인덱스 갱신 비용 발생하여 이득 없이 비용만 추가된다.  

10. 최종적으로 "인덱스 추가를 적극적으로 권장한다 / 우선순위가 낮다" 중 하나를 선택하고 근거를 작성하세요.
- 우선순위가 낮다 
- 카디널리티가 낮고, 선택도가 나쁘고, 테이블이 작아 Seq Scan이 이미 충분히 빠르기 때문에 인덱스 추가는 효과가 거의 없고 유지 비용만 증가시키기 떄문에.
