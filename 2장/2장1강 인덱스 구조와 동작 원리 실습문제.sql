...
10. 다음 질문에 답하세요.
    ...
    - 범위 비교( <, <=, >, >=, BETWEEN), ORDER BY, 앞부분이 고정된 `LIKE`, `IN` 조건 등
      정렬 순서를 활용할 수 있는 모든 비교 조건에 활용될 수 있다.
    
    Q2. B-Tree 인덱스가 범위 검색에서 검색 범위를 줄일 수 있는 이유는 무엇인가요?
    - B-Tree는 키 값이 정렬된 트리 구조라서 범위의 시작점을 log n으로 바로 찾고,
      그 지점부터 순서대로만 순회하면 되므로 조건과 무관한 데이터는 탐색 대상에서 처음부터 제외된다.
    
    Q3. 인덱스를 많이 만들수록 INSERT, UPDATE, DELETE 비용이 커질 수 있는 이유는 무엇인가요?
    - 인덱스는 테이블과 별도로 유지되는 자료구조이기 때문에, 
      데이터가 변경될 때마다 관련된 모든 인덱스도 함께 갱신해야해서 쓰기 비용이 커진다.
